...
struct ReminderOverviewTest {
  /// Tests that default frequency uses the person's setting.
  ...
  /// Tests that explicit frequency stores the frequency.
  ...
  /// Tests that past computed due date is clamped to current date.
  ...
  /// Tests that no check-in keeps existing next due date.
  ...
  /// Tests that sorted moles places nil dates at end.
  ...
  /// Tests that reminder mode Enabled sets override and active.
  ...
  /// Tests that reminder mode Disabled sets override and inactive.
  ...
  /// Tests that reminder mode Follow Default enables flag.
  ...
  /// Tests that effective reminder enabled uses person default.
  ...
  /// Tests that effective reminder enabled uses mole override.
  ...
  /// Tests that default frequency update only reapplies to follow-default moles.
  ...
  /// Tests that select previous person moves one step.
  ...
  /// Tests that select next person moves one step.
  ...
  /// Tests that sync state copies defaults.
  ...
  /// Tests that mole display frequency returns Default.
  ...
  /// Tests that mole display frequency maps enum values.
  @Test("Mole display frequency maps enum values")
  func moleDisplayFrequencyMapsEnumValues() {
    let weekly = Mole(
      name: "W", bodyPart: "Arm", isReminderActive: true, reminderFrequency: .weekly,
      nextDueDate: nil)
    weekly.followDefault = false
    let monthly = Mole(
      name: "M", bodyPart: "Arm", isReminderActive: true, reminderFrequency: .monthly,
      nextDueDate: nil)
    monthly.followDefault = false
    let quarterly = Mole(
      name: "Q", bodyPart: "Arm", isReminderActive: true, reminderFrequency: .quarterly,
      nextDueDate: nil)
  ...
  private func selectPreviousPerson(current: Person?, persons: [Person]) -> (
    selectedPerson: Person?, slideEdge: String?
  ) {
  ...
  private func selectNextPerson(current: Person?, persons: [Person]) -> (
    selectedPerson: Person?, slideEdge: String?
  ) {
  ...
  private func syncSelectionState(selectedPerson: Person?) -> (
    reminderEnabled: Bool, defaultFrequency: String
  )? {
