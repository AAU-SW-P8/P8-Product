...
/// Tests for pipeline and chart functionality.
@Suite("Pipeline")
@MainActor
struct PipelineTests {
  ...
  /// Tests that chart data follows the provided scan order.
  ...
  /// Tests that chart data returns area values for the area metric.
  ...
  /// Tests that chart data returns diameter values for the diameter metric.
  ...
  /// Tests that chart data preserves duplicate values.
  ...
  /// Tests that chart data excludes scans without a mole.
  @Test func chartDataExcludesScansWithoutMole() throws {
    let context = try makeInMemoryContext()
    let mole = makeMole(in: context)

    let day0 = Date(timeIntervalSince1970: 1_700_000_000)
    let attachedScan = attachInstance(
      to: mole, in: context, diameter: 4.0, area: 12.0, captureDate: day0)
    ...
    let points = ChartView.makeChartData(
      for: mole, metric: .area, scans: [attachedScan, orphanScan])

    #expect(points.count == 1)
    #expect(points.first?.value == Double(Float(12.0)))
  }

  /// Tests that chart data is empty for a mole with no scans.
  ...
  /// Tests that carousel selection returns image data for the correct scan.
  @Test func carouselSelectionReturnsImageDataForCorrectScan() throws {
    ...
    let scan0 = MoleScan(
      captureDate: day0, imageData: bytes0, diameter: 4.2, area: 13.8, mole: mole)
    let scan1 = MoleScan(
      captureDate: day10, imageData: bytes1, diameter: 4.8, area: 15.4, mole: mole)
    let scan2 = MoleScan(
      captureDate: day20, imageData: bytes2, diameter: 5.0, area: 16.0, mole: mole)
  ...
  /// Tests that carousel selection picks a scan matching the mole.
  ...
  /// Tests that carousel selection falls back to the first scan when no mole filter is applied.
  ...
  /// Tests that carousel selection clamps an out-of-bounds index.
  ...
  /// Tests that carousel selection returns nil for empty scans.
