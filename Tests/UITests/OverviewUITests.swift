...
final class OverviewUITests: XCTestCase {
  ...
  func testUserCanRenamePerson() {
    ...
    XCTAssertTrue(
      currentPerson.waitForExistence(timeout: 3), "Expected Alex to be selected at launch")
    ...
    XCTAssertTrue(
      nameField.waitForExistence(timeout: 3), "Name field should be visible in edit alert")
    nameField.replaceText(with: "Alex Renamed")

    editAlert.buttons["Save"].tap()

    XCTAssertTrue(
      app.staticTexts["Alex Renamed"].waitForExistence(timeout: 3),
      "Person name should update after saving")
  }
  ...
  func testUserCanCreateNewPerson() {
    ...
    XCTAssertTrue(
      addPersonButton.waitForExistence(timeout: 3), "Add person button should be visible")
    addPersonButton.tap()
    ...
    XCTAssertTrue(
      nameField.waitForExistence(timeout: 3), "Name field should be visible in add alert")
    ...
    XCTAssertTrue(
      ...
      "Newly created person should be selected")
  }
  ...
  func testCreatePersonCancelDoesNotAddPerson() {
    ...
    XCTAssertTrue(
      app.staticTexts["Alex"].exists, "Selection should remain unchanged after canceling add")
  }
  ...
  func testCreatePersonWithEmptyNameDismissesAlertAndDoesNotCreatePerson() {
    ...
    XCTAssertFalse(
      addAlert.waitForExistence(timeout: 1),
      "Add alert should dismiss after attempting to add an empty name")
    XCTAssertTrue(
      app.staticTexts["Alex"].waitForExistence(timeout: 3),
      "Selection should remain on Alex when empty name is submitted")
  }
  ...
  func testCreatePersonWithWhitespaceOnlyNameDoesNotCreatePerson() {
    ...
    XCTAssertFalse(
      addAlert.waitForExistence(timeout: 1),
      "Add alert should dismiss after attempting to add a whitespace-only name")
    XCTAssertTrue(
      app.staticTexts["Alex"].waitForExistence(timeout: 3),
      "Selection should remain on Alex when whitespace-only name is submitted")
  }
  ...
  func testRenamePersonCancelDoesNotChangeName() {
    ...
    XCTAssertFalse(
      app.staticTexts["Alex Temp"].exists, "Canceled rename should not update person name")
  }
  ...
  func testRenamePersonWithEmptyNameDoesNotChangeName() {
    ...
    XCTAssertTrue(
      app.staticTexts["Alex"].waitForExistence(timeout: 3),
      "Saving empty name should keep previous name")
  }
  ...
  func testRenamePersonWithWhitespaceOnlyNameDoesNotChangeName() {
    ...
    XCTAssertTrue(
      app.staticTexts["Alex"].waitForExistence(timeout: 3),
      "Saving whitespace-only name should keep previous name")
  }
  ...
  func testDeletePersonCancelKeepsPerson() {
    ...
    XCTAssertTrue(
      app.staticTexts["Taylor"].waitForExistence(timeout: 3), "Canceling delete should keep person")
  }
  ...
  func testDeletePersonConfirmRemovesPersonAndFallsBackSelection() {
    ...
    XCTAssertTrue(
      app.staticTexts["Alex"].waitForExistence(timeout: 3),
      "Selection should fall back after deleting selected person")
  }
  ...
  func testChangingPersonInReminderDismissesOpenDetailOrEvolutionPage() {
    ...
    XCTAssertTrue(
      detailPicker.waitForExistence(timeout: 3),
      "Detail/Evolution picker should be visible before changing person")

    Helpers.openReminderTab(in: app)
    Helpers.movePersonSelection(to: "Taylor", in: app)
    XCTAssertTrue(
      app.staticTexts["Taylor"].waitForExistence(timeout: 3),
      "Reminder selection should switch to Taylor")
    ...
    XCTAssertTrue(
      app.staticTexts["Taylor"].waitForExistence(timeout: 3),
      "Overview should reflect the newly selected person")
  }
  ...
  func testDeletedPersonPersistsAfterRelaunch() {
    ...
    XCTAssertFalse(
      rightButton.isEnabled, "Right navigation should be disabled at last available person")
...
extension XCUIElement {
  ...
  fileprivate func replaceText(with text: String) {
    tap()
