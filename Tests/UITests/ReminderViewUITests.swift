...
final class ReminderViewUITests: XCTestCase {
  ...
  private func requireMoleFrequencyButton() throws -> XCUIElement {
    ...
    guard button.waitForExistence(timeout: 3) else {
      throw XCTSkip(
        "Per-mole frequency picker is not exposed as an accessible button on this simulator runtime."
      )
    }
  ...
  private func requireDefaultFrequencyButton() throws -> XCUIElement {
    ...
    guard button.waitForExistence(timeout: 3) else {
      throw XCTSkip(
        "Default frequency picker is not exposed as an accessible button on this simulator runtime."
      )
    }
  ...
  private func setMoleReminderMode(for moleName: String, to mode: String) throws {
    ...
    guard picker.waitForExistence(timeout: 3) else {
      throw XCTSkip(
        "Per-mole enabled picker for \(moleName) is not exposed as an accessible button.")
    }
  ...
  private func chooseFrequencyOption(_ label: String) throws {
    ...
    guard option.waitForExistence(timeout: 3) else {
      throw XCTSkip(
        "Frequency option '\(label)' is not exposed as an accessible button in this runtime.")
    }
  ...
  private func selectedFrequencyValue(from accessibilityLabel: String) -> String {
    let parts = accessibilityLabel.split(
  ...
  func testReminderModeOptionsAreVisibleForMole() {
    ...
    XCTAssertTrue(
      enabledPicker.waitForExistence(timeout: 3), "The Reminder Enabled picker was not found.")
  ...
  func testDisablingDefaultReminderDisablesRemindersIfNoCustomFrequencySet() throws {
    ...
    XCTAssertFalse(
      ...
      "Expected overview reminder icon for Back Mole to disappear when default reminder is turned off and no custom frequency is set."
    )
    XCTAssertFalse(
      app.images["overviewReminderIcon_Left Arm Mole"].waitForExistence(timeout: 3),
      "Expected overview reminder icon for Left Arm Mole to disappear when default reminder is turned off and no custom frequency is set."
    )
  ...
  func testReminderControlsSyncWhenSwitchingPeople() throws {
    ...
    XCTAssertEqual(
      jordanToggle.value as? String, "1", "Expected Jordan's default reminder to be ON")
    XCTAssertEqual(selectedFrequencyValue(from: defaultFrequencyButton().label), "Weekly")

    previousPersonButton.tap()
    XCTAssertTrue(app.staticTexts["Alex"].waitForExistence(timeout: 3))
    XCTAssertEqual(
      jordanToggle.value as? String, "0", "Expected Alex's default reminder to remain OFF")
  ...
  func testDefaultReminderSettingsPersistAfterRelaunch() throws {
    ...
    XCTAssertEqual(
      ...
      "Expected default reminder OFF state to persist after relaunch")
  }
  ...
  func testChangingDefaultFrequencyDoesNotOverrideCustomMoleFrequency() throws {
    ...
    XCTAssertEqual(selectedFrequencyValue(from: firstMoleFrequencyButton().label), "Quarterly")
    XCTAssertEqual(
      selectedFrequencyValue(from: defaultFrequencyButton().label), targetDefaultFrequency)
  }
  ...
  func testDueDateIsShownAsFormattedDateWhenPresent() {
    ...
    XCTAssertTrue(
      hasDateLikeLabel, "Expected at least one date-like label in Reminder view. Labels: \(labels)")
  }
  ...
  func testDetailViewShowsDueDateSummaryForMoleWithDueDate() {
    ...
    XCTAssertTrue(
      dueSummary.waitForExistence(timeout: 3),
      "Expected due-date summary to be visible in detail view")
    XCTAssertTrue(
      dueSummary.label.contains("Next due date:"), "Expected summary to contain due-date text")
  }
  ...
  func testDetailViewHidesDueDateSummaryForMoleWithoutDueDate() throws {
    ...
    XCTAssertFalse(
      app.staticTexts["moleDetailNextDueDateSummary"].waitForExistence(timeout: 2),
      "Expected no due-date summary for mole without due date")
  }
