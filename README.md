...
- **Concurrency:** Swift Concurrency (`async/await`, `Task`) is utilised heavily within the `MoleImagePipeline` to handle SAM 3 model loading and image processing without blocking the main thread.
- **Machine Learning:** Core ML pipelines integration utilising a local SAM 3 model for semantic image segmentation.
...
│   ├── SAM3ModelLoader.swift   # Loader for SAM 3
...
│   └── models/                 # On-device Core ML Model files (.mlpackage)
│
