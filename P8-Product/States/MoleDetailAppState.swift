...
final class MoleDetailAppState {
  ...
  /// The currently active mole.
  var activeMole: Mole {
    selectionState.selectedMole ?? initialMole
  }

  /// Scans sorted descending by capture date.
  var scans: [MoleScan] {
    activeMole.scans
      .sorted { $0.captureDate > $1.captureDate }
  }

  /// All moles for the active person sorted by body part and then by name.
  ...
  func requestDeleteSelectedDetailInstance() {
    guard
      let scan: MoleScan = ImageCarousel.selectedScan(in: scans, at: selectedIndex, for: activeMole)
    else {
      return
    }
