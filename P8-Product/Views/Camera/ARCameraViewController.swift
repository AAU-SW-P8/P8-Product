...
final class ARCameraViewController: UIViewController, ARSessionDelegate {
  ...
  private func setupMeasurementDisplay() {
    ...
    measurementDisplay.backgroundColor = ARCameraConstants.Colors.overlayRed.withAlphaComponent(
      ARCameraConstants.UI.measurementDisplayBackgroundAlpha)
    ...
    displayLabel.font = .systemFont(
      ofSize: ARCameraConstants.UI.displayLabelFontSize, weight: .bold)
    ...
    confidenceLabel.font = .systemFont(
      ofSize: ARCameraConstants.UI.confidenceLabelFontSize, weight: .regular)
  ...
  private func setupCaptureButton() {
    ...
    captureButton.titleLabel?.font = .systemFont(
      ofSize: ARCameraConstants.UI.captureButtonFontSize, weight: .semibold)
  ...
  private func setupFlashlightButton() {
    ...
    flashlightButton.backgroundColor = UIColor.black.withAlphaComponent(
      ARCameraConstants.UI.flashlightButtonBackgroundAlpha)
  ...
  private func activateConstraints() {
    NSLayoutConstraint.activate([
      ...
      measurementDisplay.bottomAnchor.constraint(
        equalTo: view.safeAreaLayoutGuide.bottomAnchor,
        constant: ARCameraConstants.UI.measurementDisplayBottomPadding),
      measurementDisplay.centerXAnchor.constraint(equalTo: view.centerXAnchor),
      measurementDisplay.widthAnchor.constraint(
        equalToConstant: ARCameraConstants.UI.measurementDisplayWidth),
      measurementDisplay.heightAnchor.constraint(
        equalToConstant: ARCameraConstants.UI.measurementDisplayHeight),

      displayLabel.topAnchor.constraint(
        equalTo: measurementDisplay.topAnchor, constant: ARCameraConstants.UI.displayLabelTopPadding
      ),
      displayLabel.centerXAnchor.constraint(equalTo: measurementDisplay.centerXAnchor),

      confidenceLabel.topAnchor.constraint(
        equalTo: displayLabel.bottomAnchor, constant: ARCameraConstants.UI.confidenceLabelTopPadding
      ),
      confidenceLabel.centerXAnchor.constraint(equalTo: measurementDisplay.centerXAnchor),

      captureButton.bottomAnchor.constraint(
        equalTo: measurementDisplay.topAnchor,
        constant: ARCameraConstants.UI.captureButtonBottomPadding),
      captureButton.centerXAnchor.constraint(equalTo: view.centerXAnchor),
      captureButton.widthAnchor.constraint(
        equalToConstant: ARCameraConstants.UI.captureButtonWidth),
      captureButton.heightAnchor.constraint(
        equalToConstant: ARCameraConstants.UI.captureButtonHeight),

      cancelButton.topAnchor.constraint(
        equalTo: view.safeAreaLayoutGuide.topAnchor,
        constant: ARCameraConstants.UI.cancelButtonTopPadding),
      cancelButton.leadingAnchor.constraint(
        equalTo: view.safeAreaLayoutGuide.leadingAnchor,
        constant: ARCameraConstants.UI.cancelButtonLeadingPadding),

      flashlightButton.topAnchor.constraint(
        equalTo: view.safeAreaLayoutGuide.topAnchor,
        constant: ARCameraConstants.UI.flashlightButtonTopPadding),
      flashlightButton.trailingAnchor.constraint(
        equalTo: view.safeAreaLayoutGuide.trailingAnchor,
        constant: ARCameraConstants.UI.flashlightButtonTrailingPadding),
      flashlightButton.widthAnchor.constraint(
        equalToConstant: ARCameraConstants.UI.flashlightButtonWidth),
      flashlightButton.heightAnchor.constraint(
        equalToConstant: ARCameraConstants.UI.flashlightButtonHeight),
    ])
  }
  ...
  private func closestDepthAndConfidence(from frame: ARFrame) -> (Float, ARConfidenceLevel)? {
    ...
    for y in max(0, yStart)..<min(height, yEnd) {
      ...
      if let confBase = confidenceBaseAddress {
        confRowPtr = confBase.advanced(by: y * confidenceBytesPerRow).assumingMemoryBound(
          to: UInt8.self)
      }
      ...
      for x in stride(from: 0, to: width, by: step) {
        ...
        if depth > ARCameraConstants.Measurement.minValidDepth
          ...
          dotPath.append(
            UIBezierPath(
              ovalIn: CGRect(
                x: vx - dotSize / 2, y: vy - dotSize / 2, width: dotSize, height: dotSize)))
        }
      }
    }

    lidarPointsLayer.path = dotPath.cgPath
    lidarPointsLayer.fillColor =
      UIColor.white.withAlphaComponent(ARCameraConstants.UI.lidarDotAlpha).cgColor
  }
  ...
  private func updateMeasurementDisplay() {
    ...
    let isInRange =
      abs(currentDistance - ARCameraConstants.Measurement.targetDistance)
      <= ARCameraConstants.Measurement.distanceTolerance

    captureButton.isEnabled = isInRange
    captureButton.backgroundColor = isInRange ? .systemGreen : .systemGray
    measurementDisplay.backgroundColor =
      isInRange
      ? ARCameraConstants.Colors.overlayGreen
      : ARCameraConstants.Colors.overlayRed.withAlphaComponent(
        ARCameraConstants.UI.measurementDisplayBackgroundAlpha)
  }
  ...
  @objc private func capturePhoto() {
    ...
    let image = UIImage(
      cgImage: cgImage, scale: 1.0, orientation: Self.imageOrientation(for: interfaceOrientation))

    onCapture?(
      image, frame.sceneDepth?.depthMap, frame.sceneDepth?.confidenceMap, frame.camera.intrinsics)
  }
