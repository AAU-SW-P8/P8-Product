...
final class SAM3ImagePreprocessor {
  ...
  /// The Core Image context for rendering.
  ...
  private func makeResizedPixelBuffer(from cgImage: CGImage, size: Int) throws -> CVPixelBuffer {
    ...
    let resizedCI: CIImage = originalCI.transformed(
    ...
    -> MLMultiArray
  {
    ...
    let dataPtr: UnsafeMutablePointer<Float16> = array.dataPointer.bindMemory(
      to: Float16.self, capacity: 3 * hw)
