...
struct SAM3Models {
    ...
    static func load() async throws -> SAM3Models {
        ...
        let vision = try await loadModel(named: "SAM3.1_ImageEncoder_FP16", configuration: visionConfig)
        print("📦 Loading SAM3 text encoder…")
        let text = try await loadModel(named: "SAM3.1_TextEncoder_FP16", configuration: defaultConfig)
        print("📦 Loading SAM3 decoder…")
        let decoder = try await loadModel(named: "SAM3.1_Detector_FP16", configuration: defaultConfig)
