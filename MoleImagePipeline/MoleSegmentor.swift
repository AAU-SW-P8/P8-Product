...
class MoleSegmentor {
  ...
  func segment(image: UIImage, confidenceThreshold: Float = 0.3, nmsThreshold: Float = 1.0) throws
    ...
    let decodeTime = try clock.measure {
      decoderOutput = try decoder.run(
        visionFeatures: visionOutput, textFeatures: textPrompt.features)
    }
    ...
    guard
      let (overlay, boxes) = renderer.renderOverlay(
        on: image, detections: detections, masks: decoderOutput.masks)
    else {
      return nil
    }
    ...
    guard
      let maskOnly = renderer.renderMaskOnly(
        imageSize: image.size, detections: detections, masks: decoderOutput.masks)
    else {
      return nil
    }
