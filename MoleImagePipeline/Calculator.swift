...
class Calculator {
  ...
  func gatherMoleSamples(
    ...
    return Calculator.withLockedBuffers(depthMap: depthMap, confidenceMap: confidenceMap) {
      depth, confidence in
      self.samplePixels(
        context: context,
        depth: depth,
  ...
  func samplePixels(
    ...
    let alphaScale =
      Double(SegmentationRendererValues.maskAlphaMax) * SegmentationRendererValues.alphaByteScale
    ...
    let estimatedPixelCount = max(
      0, (bounds.maxX - bounds.minX + 1) * (bounds.maxY - bounds.minY + 1))
  ...
  static func prepareSamplingContext(
    ...
    guard let box = segmentedImage.1.first,
      let renderedMask = CalculatorHelper.renderToRGBA(image: segmentedImage.0)
    else {
      return nil
    }
