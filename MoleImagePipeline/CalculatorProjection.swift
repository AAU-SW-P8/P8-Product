...
class CalculatorProjection: Calculator {
  ...
  private func computeAreaMM2(
    from samples: Calculator.MoleSamples, _ intrinsics: (fx: Double, fy: Double)
  ) -> CGFloat {
    ...
    let areaSquareMeters =
      weightedPixelCount * (depthMeters * depthMeters) / (intrinsics.fx * intrinsics.fy)
  ...
  private func computeDiameterMM(
    from samples: Calculator.MoleSamples, _ intrinsics: (fx: Double, fy: Double)
  ) -> CGFloat {
    ...
    guard !samples.depths.isEmpty,
      samples.points.count >= LesionSizingConstants.minimumDiameterPointCount
    else {
      return 0.0
    }
