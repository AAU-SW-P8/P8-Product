...
final class SegmentationRenderer {
    ...
    func renderOverlay(
        ...
        for (detId, det) in detections.enumerated() {
            ...
            drawLabel(forDetectionId: detId, prob: det.prob, at: pixelBox, color: color, imageSize: imageSize)
        }
    ...
    private func buildTintedMaskImage(
        ...
        var minX = maskW, minY = maskH, maxX = 0, maxY = 0
        var anyPixel = false
        ...
        let cgImage = makeCGImage(from: rgbaBuffer, width: maskW, height: maskH, bytesPerRow: bytesPerRow)
        let maskBox: CGRect? = anyPixel
    ...
    private func makeCGImage(from buffer: [UInt8], width: Int, height: Int, bytesPerRow: Int) -> CGImage? {
